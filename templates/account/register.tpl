	...
	<div class="container-fluid p-0">
		<div class="row no-gutters">
			<div class="col-md-5 right-line-md">
				<p class="p-3 bottom-line m-0">{{ 'Comprá más rápido y llevá el control de tus pedidos, ¡en un solo lugar!'| translate }}</p>

				{% if account_validation == 'pending' %}
					{# Account Validation #}
					<div class="js-account-validation-pending alert p-3 top-no-line bottom-line">
					...
					<div class="p-3 font-small bottom-line">
					...
					<div class="js-resend-validation-success alert alert-success top-no-line bottom-line" style="display:none">
						<span class="m-1">{{ "¡El link fue enviado correctamente!" | translate }}</span>
					</div>
					<div class="js-resend-validation-error alert alert-danger top-no-line bottom-line" style="display:none">
				...
				{% else %}
					...
					{% embed "snipplets/forms/form.tpl" with{form_id: 'login-form', submit_custom_class: 'js-recaptcha-button btn-block btn-line', submit_prop: 'disabled', submit_text: 'Crear cuenta' | translate, data_store: 'account-register' } %}
						{% block form_body %}
					...
					{% endembed %}
				{% endif %}
				<p class="{% if account_validation == 'pending' %}text-left p-3 m-0{% else %}my-2 text-center{% endif %}">{{ '¿Ya tenés una cuenta?' | translate }} {{ "Iniciá sesión" | translate | a_tag(store.customer_login_url, '', 'btn-link ml-1') }}</p>
			</div>
		</div>
	</div>
</section>
