...
{% if not cart_page %}
  ...
  <h5 class="js-visible-on-cart-filled row no-gutters p-3 mb-0 bottom-line" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-subtotal">
  ...
  <div class="js-total-promotions text-accent">
    ...
    {% for promotion in cart.promotional_discount.promotions_applied %}
      ...
      {% else %}
        {% set id = 'all' %}
      {% endif %}
        <span class="js-total-promotions-detail-row row no-gutters p-3 bottom-line" id="{{ id }}">
          <span class="col pr-3">
...
{% if cart_page %}
  ...
  <div id="cart-sticky-summary" class="position-sticky-md cart-page-totals bottom-line mb-neg-1">
    {% if has_free_shipping_bar %}
      {# includes free shipping progress bar: only if store has free shipping with a minimum #}
    ...
    <h5 class="js-visible-on-cart-filled row no-gutters font-weight-normal mb-0 bottom-line p-3" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-subtotal">
      <span class="col-7 pl-md-0">
        {{ "Subtotal" | translate }}:
      </span>
    ...
    {% if show_calculator_on_cart %}
      <div id="shipping-cost-container" class="js-fulfillment-info js-visible-on-cart-filled js-shipping-cost-table font-weight-normal row no-gutters bottom-line p-3 mb-0" {% if cart.items_count == 0 or (not cart.has_shippable_products) %}style="display:none;"{% endif %}>
...
{% endif %}
    ...
    <div class="js-cart-total-container js-visible-on-cart-filled" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-total">
      <h2 class="row no-gutters bottom-line p-3 mb-0">
        <span class="col {% if cart_page %}pl-md-0{% endif %}">{{ "Total" | translate }}:</span>
    ...
    <div class="js-visible-on-cart-filled {% if cart_page %}row no-gutters{% endif %}" {% if cart.items_count == 0 %}style="display:none;"{% endif %}>
      ...
      {% if cart_page %}
        ...
        {% else %}
          ...
          <div class="alert alert-warning bottom-line top-no-line w-100 mb-2 text-center">
      ...
      {% else %}
        ...
        <div class="js-ajax-cart-minimum alert alert-warning bottom-line top-no-line mb-2 text-center" {{ cart.checkout_enabled ? 'style="display:none"' }} id="ajax-cart-minumum-div">
          {{ "El monto mínimo de compra es de {1} sin incluir el costo de envío" | t(cart_total | money) }}
        </div>
      ...
      {% if settings.continue_buying %}
        <div class="text-center w-100 {% if not cart_page %}mb-md-2 bottom-line-md{% endif %} pb-3">
