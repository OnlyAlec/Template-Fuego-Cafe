...
{% if use_menu %}
	<a href="#" class="js-modal-open utilities-container {% if settings.logo_position_mobile == 'center' %}right-line{% else %}left-line {% if settings.logo_position_desktop == 'center' %}left-no-line-md{% endif %}{% endif %} {% if settings.logo_position_desktop == 'center' %}right-line-md{% else %}left-line-md right-no-line-md{% endif %}" data-toggle="#nav-hamburger" aria-label="{{ 'Menú' | translate }}" data-component="menu-button">
		{% if text_utilities %}
	...
	<div id="ajax-cart" class="cart-summary" data-component='cart-button'>
		<a 
			...
			{% else %}
				href="{{ store.cart_url }}" 
			{% endif %}
			class="{% if settings.ajax_cart and template != 'cart' %}js-modal-open js-fullscreen-modal-open{% endif %} position-relative utilities-container left-line"
			>
			{% if text_utilities %}
