...
$footer_fg_color: {{ settings.footer_fg_color }};
$footer_social_bg_color: {{ settings.footer_social_bg_color }};
$footer_social_fg_color: {{ settings.footer_social_fg_color }};
$footer_link_size: {{ settings.footer_link_size }};
...
{% if settings.contactanos_colors %}
  $contactanos-background: {{ settings.contactanos_background_color }};
  $contactanos-foreground: {{ settings.contactanos_text_color }};
{% endif %}


{% if settings.icon_text_color %}
  $icono-contactanos-foreground: {{ settings.icon_text_color }};
{% endif %}

{% if settings.contactanos_font_size %}
  $contactanos-fontsize: {{ settings.contactanos_font_size }};
{% endif %}

{% if settings.laboratorio_colors %}
  $laboratorio-background: {{ settings.laboratorio_background_color }};
  $laboratorio-foreground: {{ settings.laboratorio_text_color }};
{% endif %}

{% if settings.laboratorio_font_size %}
  $laboratorio-fontsize: {{ settings.laboratorio_font_size }};
{% endif %}

{% if settings.nosotros_font_size %}
  $nosotros-fontsize: {{ settings.nosotros_font_size }};
{% endif %}

{% if settings.nosotros_seccion1_bg and settings.nosostros_seccion1_text %}
  $nosotros-seccion1-bg: {{ settings.nosotros_seccion1_bg }};
  $nosotros-seccion1-foreground: {{ settings.nosostros_seccion1_text }};
{% endif %}

{% if settings.nosotros_seccion2_bg and settings.nosostros_seccion2_text %}
  $nosotros-seccion2-bg: {{ settings.nosotros_seccion2_bg }};
  $nosotros-seccion2-foreground: {{ settings.nosostros_seccion2_text }};
{% endif %}
...
{% if settings.sm_shop_color_title %}
  $sm_shop_color_title: {{settings.sm_shop_color_title}};
{% else %}
  $sm_shop_color_title: #F6F2E9;
{% endif %}

{% if settings.sm_shop_color_text %}
  $sm_shop_color_text: {{settings.sm_shop_color_text}};
{% else %}
  $sm_shop_color_text: #F6F2E9;
{% endif %}

{% if settings.sm_shop_color_primary %}
  $sm_shop_color_primary: {{settings.sm_shop_color_primary}};
{% else %}
  $sm_shop_color_primary: #A48F7A;
{% endif %}

{% if settings.sm_shop_color_secondary %}
  $sm_shop_color_secondary: {{settings.sm_shop_color_secondary}};
{% else %}
  $sm_shop_color_secondary: #333;
{% endif %}

{% if settings.sm_shop_color_btn %}
  $sm_shop_color_btn: {{settings.sm_shop_color_btn}};
{% else %}
  $sm_shop_color_btn: #D2B897;
{% endif %}
...
{% if settings.sm_shop_bg_color %}
   $sm_shop_bg_color: {{ settings.sm_shop_bg_color }};
{% endif %}

$data_receta_fg: {{ settings.data_receta_fg }};
$data_receta_bg: {{ settings.data_receta_bg }};
$page_recetas_bg: {{ settings.page_recetas_bg }};
$page_recetas_fg: {{ settings.page_recetas_fg }};
$page_recetas_accent: {{ settings.page_recetas_accent }};
$page_recetas_box_color_text: {{ settings.page_recetas_box_color_text }};

$page_shop_bg: {{settings.page_shop_bg}};
$page_shop_img: "https:{{"page_shop_img.jpg" | static_url}}";
$page_shop_title_color: {{settings.page_shop_title_color}};
$page_shop_leyend_color: {{settings.page_shop_leyend_color}};
$page_shop_color_main: {{settings.page_shop_color_main}};
$page_shop_color_secondary: {{settings.page_shop_color_secondary}};
$page_shop_color_text: {{settings.page_shop_color_text}};
...
{# /* // Footer */ #}
  ...
  .footer-link a {
    color: $footer_fg_color;
    text-align: center;
    font-size: $footer_link_size;
    font-weight: 400;
    line-height: 24px;
  }

  .social-icons a {
      color: $footer_social_fg_color;
      border-radius: 50px;
      background: $footer_social_bg_color;
      display: flex;
      padding: 10px;
...
{# /* // Tienda Home */ #}
  .section-featured-home {
    {% if settings.sm_shop_bg_color %}
    background-color: $sm_shop_bg_color;
    {% endif %}
    padding: 20px;
  }
  ...
  .product-bg {
    background: $sm_shop_color_primary;
  }

  .product-bg-alt {
    background: $sm_shop_color_secondary;
  }
  ...
  .product-btn {
    ...
    border: 1px solid $sm_shop_color_btn;
    border-radius: 25px;
    padding-left: 30px;
    padding-right: 30px;
    padding-top: 15px;
...
{% if settings.contactanos_colors %}
  .section-contactanos {
    background-color: $contactanos-background;
    color: $contactanos-foreground;
    .btn-link {
      color: $contactanos-foreground;
    }
  }
...
{% if settings.icon_text_color %}
  .icon_contactanos_direccion {
	width : 55px;
	height : 55.5px;
  }
  
  .icon_contactanos_whatsapp {
	width : 55px;
	height : 53px;
  }
  .icon_contactanos_email{
	width : 55px;
	height : 55px;
  }
...
{% if settings.icon_text_color %}
  .icon_contactanos_color {
	fill: $icono-contactanos-foreground !important;
  }
{% endif %} 

{% if settings.contactanos_font_size %}
  .text_contactanos_size {
	font-size: $contactanos-fontsize;
  }
  @media(max-width: 767.98px) {
    .text_contactanos_size {
    font-size: 20px;
...
{% if settings.laboratorio_colors %}
  .section-laboratorio														 {
    background-color: $laboratorio-background;
    color: $laboratorio-foreground;
    .btn-link {
      color: $laboratorio-foreground;
    }
  }
  
  .section-laboratorio a {
	color: $laboratorio-foreground;
  }
  .section-laboratorio a:hover{
	color: $laboratorio-foreground;
  }
  .laboratorio-text-color{
	color: $laboratorio-foreground !important;
  }
{% endif %}

{% if settings.laboratorio_font_size %}
  .text_laboratorio_size {
	font-size: $laboratorio-fontsize !important;
  }
{% endif %} 
...
{% if settings.nosotros_font_size %}
  .text_nosotros_size {
	font-size: $nosotros-fontsize !important;
	line-height : 40px;
  }
{% endif %}

{% if settings.nosotros_seccion1_bg and settings.nosostros_seccion1_text %}
 .nosotros_seccion1_bg {
	background-color: $nosotros-seccion1-bg !important;
    padding-inline: 4%;
 }
 .nosotros-text-color{
	color: $nosotros-seccion1-foreground !important;
  }
{% endif %} 
...
{% if settings.nosotros_seccion2_bg and settings.nosostros_seccion2_text %}
 .nosotros_seccion2_bg {
	background-color: $nosotros-seccion2-bg !important;
    padding-inline: 4%;
 }
 .nosotros-text2-color{
	color: $nosotros-seccion2-foreground !important;
  }
{% endif %} 

{# /* // Recetas Home */ #}
.section-recetas-home {
  background-color: $data_receta_bg;
}
.section-recetas-home p {
  color: $data_receta_fg;
  font: 700 48px / 60px Hergon Grotesk;
  margin-bottom: 0px;
}
...
.icon-superior {
  max-height: 250px;
  max-width: 250px;
  transform: rotate(-20.022deg);
}

.icon-methods {
  border-radius: 30px;
  border: 6px solid $data_receta_fg;
...
@media(max-width: 767.98px) {
  .title {
    font: 700 40px / 45px Hergon Grotesk;
    padding-top: 50px;
  }
...
.box-content {
	border-radius: 30px;
	background: $page_recetas_accent;
	color: $page_recetas_box_color_text;
	width: 100%;
  padding-top: 3rem;
  padding-bottom: 3rem;
	padding-right: 8rem;
...
.section-recetas {
	background-color: $page_recetas_bg;
  padding: 1rem !important;
}
.section-recetas h2{
	color: $page_recetas_fg;
  font-size: 48px;
  font-weight: 700;
  line-height: 60px;
}
...
{# /* // Shop Page */ #}
.section-shop {
  background-color: $page_shop_bg;
  padding-left: 1rem;
  padding-right: 1rem;
}

.page-shop-hero {
    background-image: url($page_shop_img);
...
.page-shop-secondary {
    border-radius: 15px;
    background: $page_shop_color_secondary;
}

.page-shop-secondary img {
    width: 100%;
    max-width: 250px;
    max-height: 300px;
}
...
.page-shop-main {
    border-radius: 15px;
    background: $page_shop_color_main;
    color: $page_shop_color_text;
    height: 100%;
}
